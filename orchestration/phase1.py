"""
orchestration/phase1.py

Phase 1 of the pipeline: compile Bicep -> fetch live state -> diff -> sidecars
-> persist `reports/{label}-drift.json`. `run()` is the entry point.

This was the body of run_drift_check.py, which is now only the CLI around it.
It lived in the entry point, and orchestration/ - the layer ABOVE - had to
reach back down into it: detection.py imported `run` from it, and
reconciliation.py could only reach _attribute_orphans_to_missing_rgs through a
deferred local import, because a module-level one closed a cycle. The
dependency now points one way.
"""
