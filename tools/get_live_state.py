...
def _augment_untracked_resources(
    ...
    try:
        resources.extend(_query_backup_children(resources, sub_id, token=token))
    except Exception as e:
        logger.warning(f"Failed to query vault backup config: {e}")
    try:
        resources.extend(_query_backup_policies(resources, sub_id, token=token))
    except Exception as e:
        logger.warning(f"Failed to query vault backup policies: {e}")
    try:
...
def _shape_backup_config(vault_name: str, rg: Optional[str], payload: Dict) -> Dict:
...
def _query_backup_children(resources: List[Dict], sub_id: str, token: Optional[str] = None) -> List[Dict]:
...
def _shape_backup_policy(vault_name: str, rg: Optional[str], payload: Dict) -> Dict:
...
def _query_backup_policies(resources: List[Dict], sub_id: str, token: Optional[str] = None) -> List[Dict]:
    ...
    while the Azure-managed defaults are not false extras. A shared token may be
    passed in to avoid re-auth.
    """
    import json as _json
    import urllib.request
