...
| **There are TWO name-correspondence rules and merging them is a defect.** `smart_matching.names_plausibly_correspond` is **pairing**; `activity_log.could_be_same_resource` is **attribution**. Pinned by `tests/test_identity_matching_boundary.py` | They answer different questions, so they diverge in *both* directions. Pairing is permissive: it feeds a best-of-N selection that corrects a loose accept, whereas a false reject fabricates a `missing_in_azure`. Attribution is strict: nothing downstream corrects it, and a false accept asserts a **named person** did something they did not — issue #350, where a function app adopted an App Service's deletion event and actor because both are `Microsoft.Web/sites`. Attribution is also *looser* on segment depth, because an extension's event id names only the extension while the declared name qualifies it with its parent |
| **The pairing rule has exactly one definition, and every module deriving a static name prefix must import it** — enforced by `tests/test_deployed_resource_lookup.py`, which scans the source tree | The same `split("[")[0]` correspondence test grew **four** independent copies. Three were fixed together (#425/#426/#427); the fourth, in `tools/diff_states.py`, survived because the guard added at the time named two files rather than scanning, and had drifted further than the originals — discarding a sub-three-character prefix instead of treating it as undiscriminating, so a disk declared `d-[uniqueString()]` erased every live disk and reported the declared ones deleted. A guard scoped to the files already fixed is not a guard |
